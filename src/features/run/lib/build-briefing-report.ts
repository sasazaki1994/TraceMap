...
export function buildBriefingReport(input: BuildBriefingReportInput): string {
  ...
  return [
    ...
    "## Source Quality Notes", ...(input.sourceQuality.length?input.sourceQuality.map(q=>`- ${q.label}: ${sourceQualityGradeLabel(q.quality)} / ${sourceFreshnessLabel(q.freshness)} / ${sourceReachabilityLabel(q.reachability)}. ${q.reasons.join(' ')}`):["- No source quality notes are available yet."]),"",
