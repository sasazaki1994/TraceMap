...
| **Counterpoints** | `counterpoints` table | Each row challenges exactly one claim (`claim_id` FK). Providers may attach **one or more** counterpoints per claim. |
...
- The **mock** answer-graph provider (`src/server/analysis/providers/mock-answer-graph-provider.ts`), invoked via `createAnalysisRunFromProvider`, yields a payload that persistence writes as **two** `Claim` rows (with `graph_node_id` pointing at `node_claim_*` in **v2** `graph_json`), **one `Counterpoint` per claim**, **per-claim `Alert` rows** (`claim_id` set), plus **one answer-wide `Alert`** (`claim_id` null, `warning`). The **stub** provider omits evidence rows (empty claims/alerts) by design.
- The **openai** provider (`src/server/analysis/providers/openai-answer-graph-provider.ts`) persists **one row per model claim**, links each to supporting sources via **`claim_source_snapshots`**, and builds **graph_json v2** with **claim** nodes. Structured output may include **per-claim** `counterpoints` and `alerts`; legacy **`counterpoint_summary`** / top-level **`alert`** still map to the first claim and answer-wide alert when per-claim fields are omitted. If the model reports insufficient grounding (`sufficient_grounding: false`) or validation fails (e.g. fewer than two valid URLs), the run does not complete successfully.
- Run and share pages load these via `mapAnswerEvidenceForView` and render read-only sections in `RunResultView` (`data-testid` hooks: `run-alerts-section`, `run-alerts`, `run-alert`, …; claim-scoped alerts under each claim: `run-claim-alerts`, `run-claim-alert`, `run-claim-alert-level`, `run-claim-alert-message`; `run-claims-section`, `run-claims`, `run-claim`, `run-claim-item`, `run-claim-graph-link`, `run-counterpoints-section`, `run-counterpoint`, `run-counterpoint-item`).
